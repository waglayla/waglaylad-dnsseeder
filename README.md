...
[![GoDoc](https://img.shields.io/badge/godoc-reference-blue.svg)](http://godoc.org/github.com/Waglaylapyi/waglaylad-dnsseeder)

DNSSeeder exposes a list of known peers to any new peer joining the waglayla network via the DNS protocol.

When DNSSeeder is started for the first time, it will connect to the waglaylad node
...
disk in a json file, so on subsequent start ups the waglaylad node specified with
...
- Launch a waglaylad node for the DNSSeeder to connect to
...
```bash
$ git clone https://github.com/Waglaylapyi/waglaylad-dnsseeder $GOPATH/src/github.com/Waglaylapyi/waglaylad-dnsseeder
$ cd $GOPATH/src/github.com/Waglaylapyi/waglaylad-dnsseeder
$ go install . 
```
...
To create a working set-up where the DNSSeeder can provide IPs to waglaylad instances, set the following DNS records:
```
NAME                        TYPE        VALUE
----                        ----        -----
